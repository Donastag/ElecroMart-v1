...
const HeroImage: React.FC<HeroImageProps> = ({ productName, angleIndex, bundleId, itemId, className = "", isBackground = false }) => {
  ...
  useEffect(() => {
    ...
    const loadLocalImage = async () => {
      // Find the bundle that contains this product
      const bundle = BUNDLES.find(b => 
        b.item1.name === productName || b.item2.name === productName
      );
      
      if (bundle) {
        const item = bundle.item1.name === productName ? bundle.item1 : bundle.item2;
        const imageSrc = item.images[angleIndex] || item.images[0]; // Fallback to first image
      ...
      // If no local image found, try to use AI generation as fallback
      try {
        ...
        console.error("Hero image loading failed", err);
      } finally {
...
export const BlackNovemberHero: React.FC = () => {
  ...
  useEffect(() => {
    ...
    }, 2000); // Change product angle every 2s
